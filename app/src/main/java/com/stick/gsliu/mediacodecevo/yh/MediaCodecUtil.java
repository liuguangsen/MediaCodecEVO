package com.stick.gsliu.mediacodecevo.yh;
...
public class MediaCodecUtil {
    ...
    public boolean isShowSurface = true;
    // 需要解码的类型
    public String MIME_TYPE = "video/avc"; // H.264 Advanced Video
...
//                "width = [" + width + "], height = [" + height + "]");
    ...
    private int setExtraInfo(MediaFormat mediaFormat) {
...
//        mediaFormat.setInteger(MediaFormat.KEY_COLOR_FORMAT,MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Flexible);
        ...
        try {
            ...
            Boolean bSupport = Boolean.TRUE;
            Boolean bResolution = Boolean.TRUE;
            if (CheckSupport) {
...
//        }
        ...
        Log.i(TAG, "check for getAacFile.");
        try {
            //开始解码
            mCodec.start();
        } catch (Exception ex) {
            Log.e(TAG, "Codec getAacFile Error.");
            ex.printStackTrace();
            if (mCodec != null) {
                mCodec.release();
            }
...
//                return false;
    ...
    //解码帧
    public boolean onFrame(byte[] buf, int offset, int length, MediaCodec.BufferInfo info) {
        //检查数据帧
        if (mCodec == null) {
            return false;
        }
        ...
        if (inputBufferIndex >= 0) {
            ...
            //解码
            mCodec.queueInputBuffer(inputBufferIndex, 0, length, info.presentationTimeUs, info.flags);
            mCount++;
        } else {
